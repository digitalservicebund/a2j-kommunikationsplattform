...
describe("VerfahrenCountInfo", () => {
  ...
  describe("when filters ARE NOT applied", () => {
    ...
    describe("alerts", () => {
      ...
      it("shows no alert when count is exactly 50", () => {
        renderComponent(50, false);
        expect(queryByRole("alert")).not.toBeInTheDocument();
      });

      it("shows too many results alert when count is exactly 51", () => {
        renderComponent(51, false);
        expect(getByText(alerts.TOO_MANY_RESULTS_TITLE)).toBeInTheDocument();
      });
    });
  });
  ...
  describe("when filters ARE applied", () => {
    ...
    describe("alerts", () => {
      it("shows no results alert when count is 0", () => {
        renderComponent(0, true);
        expect(
          getByText(alerts.NO_VERFAHREN_FOUND_WITH_FILTERS_TITLE),
        ).toBeInTheDocument();
        expect(
          getByText(alerts.NO_VERFAHREN_FOUND_WITH_FILTERS_MESSAGE),
        ).toBeInTheDocument();
      });
